...
If an original garment size tag is visible, ALWAYS preserve the exact printed size exactly as shown on the garment tag, including sizes such as 0XL. Never convert 0XL to XL or any other size in the title or description. If the marketplace structured size field does not offer 0XL, use the closest available structured option only for that field, but clearly keep 0XL as the actual tag size in the title and description. If no original garment size tag is visible, use clear garment measurements shown in the photos to estimate the most likely standard size based on garment type and gender. Return the estimated standard size in the size field and clearly state "Approximate size based on measurements; no size tag visible" in the description. Never present an estimated size as a tag size. If there are not enough clear measurements to make a reasonable size estimate, leave the size blank.
...
"size": "If a size tag is visible, use the size EXACTLY as printed. If no size tag is visible but measurements clearly support an approximate standard size, return ONLY the standard size value such as S, M, L, XL, 10, 12, 32x30, etc. Do NOT include Approx., Approximate, Estimated, or Size in this field. If measurements are insufficient or unclear, use an empty string.",


"size_estimated": "Return true ONLY when size is estimated from measurements because no size tag is visible. Return false when size is read from a visible size tag or when no estimated size is provided.",
